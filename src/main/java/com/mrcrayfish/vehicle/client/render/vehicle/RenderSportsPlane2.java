...
import com.mrcrayfish.vehicle.entity.vehicle.EntitySportsPlane2;
 ...
 * Author: MrCrayfish
 */
public class RenderSportsPlane2 extends AbstractRenderVehicle<EntitySportsPlane2>
{
    @Override
    public void render(EntitySportsPlane2 entity, float partialTicks)
    {
        this.renderDamagedPart(entity, SpecialModels.SPORTS_PLANE_BODY_2.getModel());
        ...
        GlStateManager.pushMatrix();
        {
            GlStateManager.translate(0, 0, -22.2 * 0.0625);
            this.renderDamagedPart(entity, SpecialModels.SPORTS_PLANE_TALL_2.getModel());
        }
        GlStateManager.popMatrix();
    ...
    private void renderWheel(EntitySportsPlane2 vehicle, float offsetX, float offsetY, float offsetZ, float legRotation, float partialTicks)
    {
        GlStateManager.pushMatrix();
        {
    ...
    @Override
    public void applyPlayerModel(EntitySportsPlane2 entity, EntityPlayer player, ModelPlayer model, float partialTicks)
    {
    ...
    @Override
    public void applyPlayerRender(EntitySportsPlane2 entity, EntityPlayer player, float partialTicks)
    {
        ...
        if(index != -1)
        {
            ...
            Vec3d seatVec = null;
            if (index == 0){
                seatVec = seat.getPosition().addVector(0, properties.getAxleOffset() + properties.getWheelOffset(), 0).scale(properties.getBodyPosition().getScale());
            }
            if (index == 1){
                seatVec = seat.getPosition().addVector(8, properties.getAxleOffset() + properties.getWheelOffset(), 0).scale(properties.getBodyPosition().getScale());
            }
            if (index == 2){
                seatVec = seat.getPosition().addVector(16, properties.getAxleOffset() + properties.getWheelOffset(), 0).scale(properties.getBodyPosition().getScale());
            }
            if (seatVec != null){
