...
public abstract class EntityPoweredVehicle extends EntityVehicle implements IInventoryChangedListener
{
    ...
    public void onUpdateVehicle()
    {
        ...
        boolean nonada = false;
        if (!list.isEmpty())
        {
            for(Entity entity : list)
            {
            	nonada = false;
                if (entity instanceof EntityVehicleTrailer){
                    nonada = true;
                }
                if (nonada == false) {
        ...
        List<Entity> list2 = this.world.getEntitiesInAABBexcluding(this, this.getEntityBoundingBox(), entity -> entity instanceof EntityLivingBase);
        nonada = false;
        boolean danioauto = false;
        ListIterator<Entity> iter = list2.listIterator();
        if (!list2.isEmpty())
        {
            while(iter.hasNext()){
                Entity iterentity = iter.next();
                for(Entity passenger : this.getPassengers()){
                    if (iterentity.getUniqueID() == passenger.getUniqueID()){
                        iter.remove();
                    }
                }
            }
            for(Entity entity : list2)
            {
                ...
                if (entity instanceof EntityVehicle){
                	if (!(entity instanceof EntityBumperCar)){
                		this.applyDamageEntity(this, this.getSpeed());
                		danioauto = true;
                		nonada = false;
                	}
                }
                    ...
                    if (this.getSpeed() >= 3){
                        ...
                        this.applyDamageEntity(entity, this.getSpeed());
                        for(Entity passenger2 : this.getPassengers()){
                            this.applyDamageEntity(passenger2, this.getSpeed());
                        }
    ...
    public void applyDamageEntity(Entity entity, float speed)
    {
        entity.attackEntityFrom(DamageSource.GENERIC, 1 * speed / 2);
    }
